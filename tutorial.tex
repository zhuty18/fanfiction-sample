...
\begin{sloppypar}
    ...
    本文为懂得{\LaTeX}基础命令、语法，准备以此为排版工具，制作\textbf{书籍类同人本}的玩家撰写。考虑了等宽文字与不等宽文字（通常就是中英文）掺杂的情况。有少量场景与操作系统有关，本文暂只包括Windows下的处理方案。

    推荐新手使用在线工具，因此其中不包含{\LaTeX}安装等入门内容。如果想要安装在本地的话，这部分教程在网上已经很多，不需要我再抄一遍。

    我创立了一个模板，欢迎直接\jumpto{https://github.com/zhuty18/fanfiction-sample}{下载使用}。如果本文或者本模板对你出本有帮助，你出本时感谢我一句也挺好的。
    ...
    \jumpto{https://cn.overleaf.com/}{\textit{Overleaf}}是{\LaTeX}的在线IDE，全网页端，只需注册一个账号就可免费使用。使用时，在工作区左侧先点击目录（Menu），将编译引擎换成{\XeLaTeX}（\texttt{XeLaTeX}）或{\LuaLaTeX}（\texttt{LuaLaTeX}），才可正常处理中文。拼写检查系统不支持中文，建议关掉，不然中文全篇都是拼写警告。
    ...
    外语字体可在\jumpto{https://www.1001fonts.com/}{1001 Fonts}上挑选。我推荐\jumpto{https://www.1001fonts.com/eb-garamond-font.html}{EB Garamond}，该字体实现了拉丁、希腊、西里尔字母的全覆盖，用起来很省心。
...
\end{lstlisting}
    ...
    另外，也可以使用\jumpto{https://fontdrop.info}{FontDrop!}网站解析单个字体文件，获得字体名。解析样例字体\textit{EBGaramondSC12-Regular.otf}时结果如下。
...
\end{lstlisting}
    ...
    \lstinputlisting[style=TeX,caption=外语与中文使用同一字体]{codes/font-8.tex}
    ...
    尾注标题默认影响\texttt{\textbackslash{}leftmark}（该命令的用处见\textit{\nameref{hf}}），可以用以下命令规避。
    ...
    因为\jumpto{https://typst.app/}{\textit{Typst}}还是太菜了。
