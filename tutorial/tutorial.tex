...
\begin{sloppypar}
    ...
    我创立了一个模板，欢迎直接\jumpto{https://github.com/zhuty18/fanfiction-sample/releases/latest}{下载使用}。如果本文或者本模板对你出本有帮助，你感谢我一句也挺好的。
    ...
    \jumpto{https://cn.overleaf.com/}{\emph{Overleaf}}是{\LaTeX}的在线IDE，全网页端，只需注册一个账号就可免费使用。使用时，在工作区左侧先点击目录（Menu），将编译引擎换成{\XeLaTeX}（\texttt{XeLaTeX}）或{\LuaLaTeX}（\texttt{LuaLaTeX}），才可正常处理中文。拼写检查系统不支持中文，建议关掉，不然中文全篇都是拼写警告。
    ...
    如果找一些专业的中文排版攻略读（比如\jumpto{https://www.w3.org/TR/clreq/}{中文排版需求}，\jumpto{https://www.thetype.com/2020/01/16565/}{中文排版网格系统的五大迷思}），那么就会意识到，文字排版这件事其实非常细碎繁杂，需要关注的细节极多。
    ...
    \lstinputlisting[style=TeX,caption={自定义字号}]{codes/fontsize-1.tex}
    ...
    \emph{TeXLive}自带字库中包含\emph{Fandol}系列，各平台均可\texttt{{[}fontset=fandol{]}}加载全套\emph{Fandol}字体配置，简单达到跨平台统一。但是它只含GB2312内的字符，可能会出现缺字。如果出现这种问题的话，还是只能自行配置字体。

    \section{常用字体获取}

    外语字体可在\jumpto{https://www.1001fonts.com/}{1001 Fonts}上挑选。我推荐\jumpto{https://www.1001fonts.com/eb-garamond-font.html}{EB Garamond 12}，该字体实现了拉丁、希腊、西里尔字母的全覆盖，用起来很省心。

    思源系列开源，这是官方地址：\jumpto{https://github.com/adobe-fonts/source-han-serif/releases/latest}{思源宋体}，\jumpto{https://github.com/adobe-fonts/source-han-sans/releases/latest}{思源黑体}。

    方正系列官网需要登录等操作，这里给出第三方下载链接：\jumpto{https://www.fonts.net.cn/font-31610316242.html}{方正书宋}、\jumpto{https://www.fonts.net.cn/font-31607222283.html}{方正楷体}、\jumpto{https://www.fonts.net.cn/font-31602268591.html}{方正仿宋}、\jumpto{https://www.fonts.net.cn/font-31609167689.html}{方正黑体}。不确定网站可靠性，链接或许会失效。请注意，在找方正字体文件时请一定找GBK后缀（书宋字体文件8.87M），而非GB2312或简体后缀（书宋字体文件2.63M），预防字体缺字。
    ...
    得出的结果中包含字体名，这里使用开源字体\emph{Vollkorn}系列举例，\texttt{fc-list\ \textbar{}\ Select-String\ "Vollkorn"}得到的结果如下

    \begin{lstlisting}[title=\emph{Vollkorn}查询结果]
...
\end{lstlisting}
    ...
    另外，也可以使用\jumpto{https://fontdrop.info}{FontDrop!}网站解析单个字体文件，获得字体名。解析样例字体\emph{EBGaramondSC12-Regular.otf}时结果如下。
...
\end{lstlisting}
    ...
    \lstinputlisting[style=TeX,caption={路径法配置主字体}]{codes/font-11.tex}
    ...
    \lstinputlisting[style=TeX,caption=安装法配置主字体]{codes/font-12.tex}
    ...
    第三步，用\jumpto{https://fontdrop.info}{FontDrop!}网站解析单个字体文件，获得字体名，和\texttt{fc-list}里获得的可能会不一样。

    第四步，在字体专属名外面加个中括号。别问为什么，我也不懂，总而言之亲测有时候是有效的。

    \lstinputlisting[style=TeX,caption=找不到字体的处理方案]{codes/font-9.tex}
    ...
    \begin{minipage}[c][3\baselineskip]{30\ccwd}
        \centering\fontspec{Vollkorn}\Huge f i l fi fl ff
    \end{minipage}
    ...
    可以用\jumpto{https://fontdrop.info}{FontDrop!}这个网站来查看字体内连字。有些字体虽然设计了连字，但并没有正确标注，导致这些连字虽然存在但却无法使用。面对这种情况，可以用\jumpto{https://fontforge.org/en-US/}{FontForge}软件进行\jumpto{https://fontforge.org/docs/tutorial/editexample4.html}{手动标注}。
    ...
    中文的可选字体其实不少（\jumpto{https://drxie.github.io/OSFCC/}{开源字体可以见此名单}）。从各方面收集到的资料来看，方正系的字体适用范围最广、知名度最高，设计应该是最佳的。
    ...
    以下样例中，中文使用思源宋体Regular；英文字体均是从网站\jumpto{https://www.1001fonts.com}{1001 Fonts}下载的免费商用字体，全部使用Regular字重的默认配置。

    \begin{adjustwidth}{2em}{2em}
        \large\blankpar

        \fontspec{Source Han Serif CN}思源宋体\testtext
    ...
    \begin{adjustwidth}{2em}{2em}
        ...
        \fontspec{Source Han Serif CN}[Scale=0.9,LetterSpace=-5]思源宋体\testtext
    ...
    方正系列虽然覆盖了外语的字符，但并没有做对应的设计（很多被认为是抄的），基础参数也很不合理。四种免费字体内除ASCII外的全部字符，均与中文字符等宽，不适合用于显示外语。因此极不建议将其作为\texttt{fontspec}包配置的字体。例如，这是思源宋体中的{\fontspec{Source Han Serif CN} café}，这是方正书宋中的{\fontspec{FZShuSong-Z01} café}，这是EB Garamond 12中的{\fontspec{EB Garamond 12 Regular} café}。
    ...
    \lstinputlisting[style=TeX,caption={不自动插入空白页}]{codes/page-1.tex}

    相对的，如果在使用了\texttt{openany}参数后，希望某页在右页上开始，则可以使用\texttt{\textbackslash{}cleardoublepage}命令。该命令会自动插入空白页，使后续内容从下一个右页铺开。

    若希望后文在左页上开始，可以使用以下写法：

    \lstinputlisting[style=TeX,caption={在左页展开后续}]{codes/page-2.tex}

    \section{手动空白页}

    一页强制插入的白纸包含若干要素，建议直接写成命令，方便使用。

    \lstinputlisting[style=TeX,caption={手动插入空白页}]{codes/blankpage.tex}
    ...
    \lstinputlisting[style=TeX,caption={设置脚注字体}]{codes/note-3.tex}
    ...
    尾注标题默认影响\texttt{\textbackslash{}leftmark}（该命令的用处见\emph{\nameref{hf}}），可以用以下命令规避。
    ...
    \lstinputlisting[style=TeX,caption={手动设置索引中的标题名称}]{codes/toc-5.tex}
    ...
    绝大多数中文标点都能在{\LaTeX}中按标准方式渲染，但是破折号基本都不会和标准使用方法一样，等同于两个字宽。这与字体和连字均有关系，\jumpto{https://github.com/CTeX-org/ctex-kit/issues/382}{有兴趣可以阅读这篇帖子}。想完美解决很复杂，但是想简单解决真的很简单。
    ...
    \lstinputlisting[style=TeX,caption={\texttt{ulem}装饰}]{codes/ulem-1.tex}

    \input{codes/ulem-1.tex}

    注意，\texttt{ulem}包会默认将\texttt{\textbackslash{}emph}命令的效果替换为下划线，每强调一次，强调内容下方就会多一条线。可以在引入时规避。

    \lstinputlisting[style=TeX,caption={不允许\texttt{ulem}改变强调格式}]{codes/ulem-0.tex}
    ...
    \lstinputlisting[style=TeX,caption={定义颜色}]{codes/color-1.tex}

    用\texttt{\textbackslash{}color}设置字体颜色。

    \lstinputlisting[style=TeX,caption={使用颜色}]{codes/color-2.tex}

    用\texttt{\textbackslash{}colorbox}绘制彩色背景盒子。

    \lstinputlisting[style=TeX,caption={彩色背景盒子}]{codes/color-3.tex}
    ...
    单色emoji的使用方法近似于一般文字。\emoji{"1F407}上色后{\color{pink}即为\emoji{"1F407}}。{\checkmark}则为\emoji{"2714}放入彩色背景盒子后得到的。

    加载单色emoji字体后，用\texttt{\textbackslash{}char}或\texttt{\textbackslash{}symbol}输入编码使用。emoji的unicode编码可以在\jumpto{https://www.emojiall.com/}{emoji字典}上查询。

    典型的单色emoji字体如\jumpto{https://www.webfontfree.com/cn/download/Symbola}{Symbola}。

    \lstinputlisting[style=TeX,caption={单色emoji}]{codes/emoji-1.tex}

    \subsection{多色emoji}

    多色emoji使用\texttt{emoji}包处理。\textbf{只能使用\LuaLaTeX（\texttt{LuaLaTeX}）编译}。

    典型的多色emoji字体如\jumpto{https://github.com/jdecked/twemoji/releases/latest}{Twemoji}。

    \lstinputlisting[style=TeX,caption={多色emoji}]{codes/emoji-2.tex}
    ...
    因为\jumpto{https://typst.app/}{\emph{Typst}}还是太菜了。
    ...
    \jumpto{https://ask.latexstudio.net/LearnLaTeX/}{\emph{LaTeX知识库}}是一套非常基础的文档，对于各个基本功能都进行了详细讲解。适于完整阅读，以初步了解{\LaTeX}。

    \jumpto{https://ctan.math.utah.edu/ctan/tex-archive/info/lshort/chinese/lshort-zh-cn.pdf}{\emph{lshort-zh-cn}}是真正意义上的{\LaTeX}介绍文档\emph{lshort}的中文翻译版本。内容翔实丰富，基本覆盖了一切用户所需要知道的内容。适于想要认真使用{\LaTeX}的用户仔细阅读。

    \jumpto{https://www.overleaf.com/learn}{\emph{Overleaf Document}}是一套\textbf{纯英文}的进阶文档，基本涵盖了所有用得上的功能。适于比较着急的人，针对性阅读能解决自己当下问题的章节段落即可。

    \emph{{\TeX}book}是高德纳写的{\TeX}指南，\jumpto{https://github.com/seuliang/the-texbook-cn}{\emph{有中文翻译}}。建议至少读完前言和第一章“此名有诗意”，可以更好地理解{\TeX}的功能和目的；继续阅读，则能学到很多不知道用不用得上的知识。但，我建议等{\LaTeX}不够你玩了的时候，再进行深入学习。
