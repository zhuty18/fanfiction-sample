...
\texttt{\textbackslash costomizetitlepage}，启用代表使用独立扉页设计，禁用则使用\LaTeX 内置的扉页。

独立扉页文件为\texttt{titlepage.tex}，模版自带了一版设计。

\texttt{\textbackslash bleed}，启用代表增设出血（上下左右各3mm），禁用则代表按A5（148*210mm）输出\textbf{效果图}，不适宜直接用于打印。

\section{可用命令}

\texttt{\textbackslash blankpar} 此处为空白段

\texttt{\textbackslash blankpage} 插入空白页

\texttt{\textbackslash storyend[]} 完结，可选参数为打印内容，默认为“完”，格式如下

\storyend

\texttt{\textbackslash splitdot} 插入1字宽的分隔号{\splitdot}

\texttt{\textbackslash chsline}插入2字宽的破折号{\chsline}

\texttt{\textbackslash tempsymbol}插入摄氏度符号{\tempsymbol}

\texttt{\textbackslash prenotes{标题}}一种和\texttt{\textbackslash section*}不太一样的无序号小标题
