...
这只是一个模板，\href{https://zhuty18.github.io/docs/\%E5\%90\%8C\%E4\%BA\%BA\%E6\%96\%87LaTeX\%E6\%8E\%92\%E7\%89\%88\%E6\%95\%99\%E7\%A8\%8B}{\underline{点击此处}}跳转{\LaTeX}同人本排版教程。

\blankline

在禁用freefont时，本模板会使用ctex包的原生字体，能支持Overleaf在线预览。

\begin{center}
    {\bfseries 但是很丑。}
\end{center}

更漂亮的版本请参见样例pdf\endnote{即本模板配置好字体后编译得到的结果}。或将本模板下载到本地，配置好字体后启用freefont。
...
乱数假文（Lorem Ipsum）即排版时使用的无意义填充文本。本模板内含“中文与拉丁字母混合排版”，“纯中文排版”和“纯拉丁字母排版”。

\chapter{混合排版}

由于目前没有中英文混合的Lorem Ipsum包\footnote{对此深感遗憾}可用，直接引用了一篇科普来作为中英文混合的内容填充样例\endnote{本文来源于https://cn.lipsum.com/}。
...
Aenean dapibus tempor urna, id mollis nibh commodo vitae. Cras a accumsan enim, vel ultrices ipsum. Sed ullamcorper elit sit amet velit interdum sollicitudin. Donec consequat purus hendrerit, ornare sem at, mollis lectus. Etiam consectetur ipsum ac nibh ornare vehicula. Integer eu quam nibh. In nisl lectus, ullamcorper sed pharetra eu, vehicula a nisl. Fusce euismod quam lacus, a blandit sapien tempor lacinia. Vivamus finibus fermentum magna ut ornare. Morbi semper ligula leo, id condimentum libero rutrum id.
...
main.tex第8-15行，定义了一些基础信息，包括标题、作者、CP，视情况修改即可。

\section{基础定义}

main.tex第3-6行，使用定义的方式进行了基础定义。

\textbackslash freefont，启用代表使用免费商用字体（需要独立安装）。本模板使用的英文字体均可在1001Font（\underline{https://1001fonts.com}）上获取，中文字体仅限于方正书宋、方正黑体、方正楷体、方正仿宋、思源宋体五种。思源黑体也为免费商用字体，但本模板没有使用。

\textbackslash costomizetitlepage，启用代表使用独立扉页设计（见titlepage.tex，模版自带了一版设计），禁用则使用\LaTeX 内置的扉页。

\textbackslash bleed，启用代表增设出血（上下左右各3mm），禁用则代表按A5（148*210mm）输出\textbf{效果图}。

\chapter{进阶设置}

修改formats.tex以改动模板，适配自己的需求。

如果对其中一些内容感到困惑，或者发现并不能完全满足自己的需求，请阅读排版教程，\href{https://zhuty18.github.io/docs/\%E5\%90\%8C\%E4\%BA\%BA\%E6\%96\%87LaTeX\%E6\%8E\%92\%E7\%89\%88\%E6\%95\%99\%E7\%A8\%8B}{\underline{点击此处}}跳转。
